...
rho_c = 3*H_0**2 / (8.0*np.pi*G) # critical density
c = 3.75 # Concentration parameter
mp = const.m_p
mp = mp.to(u.g)
...
def nh(rvir, B):
    ...
    if len(B)==0:
        ...
        NH = (np.sqrt(np.pi)*(1/r_c**2.0)**(-3.0*beta/2.0)*(B**2.0+r_c**2.0)**(1/2. - 3*beta/2.) * gamma(-0.5 + 3*beta/2.))/(2*gamma(3*beta/2.))
    return A*NH*rho/mp
